		...
		<table id="listarUsuarios" class="table table-striped table-bordered" cellspacing="0" width="100%">
			...
			if (!empty($users)) :
				foreach ($users as $user) :
					?>
					<tr>
						<td class="text-center"><?= $user["tx_nome"]; ?></td>
						<td class="text-center"><?= $user["tx_email"]; ?></td>
						<td class="text-center"><?= (!$user["nu_telefone"] ? '-' : $user["telefone"]); ?></td>
						<td class="text-center"><?= ($user["tx_status_2"] == 1 ? "<span style='font-size: 15px;' class='badge badge-success'>ATIVO</span>" : "<span style='font-size: 15px;' class='badge badge-danger'>INATIVO</span>"); ?></td>
						<td class="text-center text-uppercase" style="width: 80px;">
							<div class="btn-group">
								<button class="btn btn-primary text-uppercase">
									<a href="<?= base_url() . 'editarUsuario?id=' . $user["id_usuario"]; ?>">Editar</a>
								</button>
							</div>
						</td>
					</tr>
